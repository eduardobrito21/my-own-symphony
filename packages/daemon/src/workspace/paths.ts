...
import { sanitizeIdentifier, type IssueIdentifier } from '../types/index.js';
 ...
 * stack trace.
 */
export function workspacePathFor(root: string, identifier: IssueIdentifier): string {
  const absoluteRoot = resolvePath(root);
  const key = sanitizeIdentifier(identifier);
  const candidate = resolvePath(absoluteRoot, key);
